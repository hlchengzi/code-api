...
public class UrlFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object object) throws IllegalArgumentException {
        ...
        //todo 可以使用布隆过滤器对url进行检验过滤
        //登录和群聊注册时候需可以访问,不会对此url路径进行拦截，其余的属于系统内调用的，则需要拦截，对url进行检验
        if (requestUrl.contains("/login") || requestUrl.contains("/groupChat")|| requestUrl.contains("register") || requestUrl.contains("getMyId")||requestUrl.contains("passenger")||requestUrl.contains("driver")){
            return null;
        }
