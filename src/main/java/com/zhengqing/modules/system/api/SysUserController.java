...
public class SysUserController extends BaseController {
    ...
    @ApiOperation(value = "保存系统管理-用户基础信息表", httpMethod = "POST", response = ApiResult.class)
    // groups和默认校验同时应用 - 没有groups的属性和有groups的属性要想同时校验，则必须在value数组中同时指明，启动没有groups的属性通过Default.class来指定
    public ApiResult register(@RequestBody @Validated User input) {
        if(input.getId()!=null){
            return ApiResult.fail("用户id已经存在，请重新输入");
        }
        Integer id = userService.save(input);
        return ApiResult.ok("保存系统管理-用户基础信息表成功", id);
    }
    ...
    @PostMapping(value = "/getMyId", produces = "application/json;charset=utf-8")
    @ApiOperation(value = "获取当前登录用户的id", httpMethod = "GET", response = ApiResult.class)
    public ApiResult getMyId() {
        UserDetails userDetails = (UserDetails) SecurityContextHolder.getContext().getAuthentication() .getPrincipal();
        String username = userDetails.getUsername();
        User user = userMapper.selectUserByUsername(username);
    return ApiResult.ok("获取当前登录用户的id", user);
    }
}
